...
public static class MeshUtils {
    ...
    public static Mesh CreateCircleMesh(float radius, int segments)
    {
        ...
        {
            uvs[i] = new Vector2(vertices[i].x / radius * 0.5f + 0.5f, vertices[i].y / radius * 0.5f + 0.5f);
        }
    ...
    public static void AddCircleToMesh(Mesh mesh, Vector3 center, float radius, int segments/*, Quaternion orientation*/)
    {
        // Get existing mesh data
        List<Vector3> vertices = new List<Vector3>(mesh.vertices);
        List<int> triangles = new List<int>(mesh.triangles);

        int startIndex = vertices.Count; // Index of the first new vertex
        float angleStep = 360f / segments;

        // Add center vertex
        vertices.Add(center);

        // Add surrounding vertices and align them using the orientation
        for (int i = 0; i < segments; i++)
        {
            float angle = Mathf.Deg2Rad * (i * angleStep);
            Vector3 point = new Vector3(Mathf.Cos(angle) * radius, Mathf.Sin(angle) * radius, 0); // Local 2D circle

            // Rotate and translate the point into world space
            point = /*orientation **/ point + center;

            vertices.Add(point);

            // Create triangles connecting this segment
            if (i > 0)
            {
                triangles.Add(startIndex);          // Center vertex
                triangles.Add(startIndex + i);     // Current vertex
                triangles.Add(startIndex + i + 1); // Next vertex
            }
        }

        // Connect the last segment to the first
        triangles.Add(startIndex);             // Center vertex
        triangles.Add(startIndex + segments);  // Last vertex
        triangles.Add(startIndex + 1);         // First outer vertex
    ...
    public static void AddCircleToSingleMesh(ref List<Vector3> vertices, ref List<int> triangles, ref List<Vector2> uvs, Vector3 position, float radius, int segments)
    {
        ...
        for (int i = 0; i <= segments; i++) // <= to close the loop
        {
            ...
            // Create triangles (skip first index)
            if (i > 0)
            {
                triangles.Add(vertexStartIndex);
                triangles.Add(vertexStartIndex + i);
